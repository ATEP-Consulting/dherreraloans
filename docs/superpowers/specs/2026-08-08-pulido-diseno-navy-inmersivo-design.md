...
2. **Header fijo con estado:** el header (con TopStrip) pasa a `position: fixed`; transparente integrado con el hero en top 0, y con `hdr-solid` (script del layout) fondo paper, texto ink, logo oscuro (`logo.png`) y franja superior plegada. Colores vía vars `--hfg/--hfg-mut/--hbr` en `.site-header`. **Afinado tras reporte de temblor al scrollear:** el estado usa **histéresis 90/30 px** (un umbral único alternaba varias veces por gesto de trackpad), la franja **colapsa con transición** `grid-template-rows: 1fr→0fr` con su padding en un nieto (con `display:none` la fila del logo saltaba 43 px de golpe) y los logos hacen **cross-fade** por opacidad en vez de swap de `display`. Medido: 1 cambio de estado por gesto, colapso 43→0 px animado, CLS de scroll 0,005, marquesina estable (desviación 0).
