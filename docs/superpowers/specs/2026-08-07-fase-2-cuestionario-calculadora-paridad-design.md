...
3. **Progreso**: «PASO X DE Y» (enmienda 2026-08-07: implementado sin el prefijo «GET A QUOTE ·» — el eyebrow del hero de `/quote` ya dice «Get a Quote» encima del quiz, repetirlo en el label de progreso era redundante) en uppercase micro + barra hairline 2px con relleno navy bajo el encabezado del quiz. Y se recalcula por flujo y respuestas (los saltos condicionales cambian el total). Barra `aria-hidden`; el texto es el accesible.
