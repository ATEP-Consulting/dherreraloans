...
import { PHONE_DISPLAY, PHONE_TEL, whatsAppHref } from '@/lib/site';
...
const plateClass =
  'flex flex-col gap-2 border border-ink bg-white p-6 transition hover:bg-sand';
...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const emailValue = t('email.value');
      ...
      <section>
        <Container className="px-5 py-10 lg:px-[72px] lg:py-16">
          <div className="grid gap-5 lg:grid-cols-3 lg:gap-6">
            ...
            <a href={`mailto:${emailValue}`} className={plateClass}>
              <Eyebrow>{t('email.label')}</Eyebrow>
              <span className="break-all font-display text-h3 font-light text-ink">{emailValue}</span>
            </a>
